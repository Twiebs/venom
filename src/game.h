inline void MoveCameraWithFPSControls(GameMemory *memory, Camera *camera)
{
	...
	if (input->isKeyDown[KEYCODE_A]) dp -= Normalize(Cross(camera->front, V3(0.0f, 1.0f, 0.0f))); 
	if (input->isKeyDown[KEYCODE_D]) dp += Normalize(Cross(camera->front, V3(0.0f, 1.0f, 0.0f)));
	...
	//dp = Normalize(dp);
	dp *= memory->gameState.deltaTime * speed;
	camera->position += dp;

	camera->yaw -= input->cursorDeltaX * 0.01f;
	camera->pitch += input->cursorDeltaY * 0.01f;
	camera->pitch = Clamp(camera->pitch, -(PI32*0.5f - (1.0f*DEG2RAD)), PI32*0.5f - (1.0f*DEG2RAD));
}
